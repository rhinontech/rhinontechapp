...
export default function Inbox() {
  ...
  const fetchEmails = useCallback(async () => {
    ...
    try {
      ...
      setEmails(data);
      setSelectedId((current) => (data.some((email: EmailItem) => email.id === current) ? current : data[0]?.id || null));
    } catch (err) {
  ...
  const fetchEmail = useCallback(
    async (id: string) => {
      try {
        const response = await fetch(`${API_URL}/inbox/${id}`, { headers: authHeaders });
        const data = await response.json();
        if (!response.ok) throw new Error(data.message || "Could not load email");
        setSelectedEmail(data);
        setEmails((items) => items.map((item) => (item.id === id ? { ...item, isRead: true } : item)));
      } catch (err) {
  ...
  const patchEmail = async (id: string, payload: Record<string, unknown>) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || "Could not update email");
      ...
      if (payload.folder && payload.folder !== folder) {
        ...
        setEmails((items) => items.map((item) => (item.id === id ? data : item)));
        setSelectedEmail((current) => (current?.id === id ? { ...current, ...data } : current));
      }
    } catch (err) {
  ...
  const sendReply = async () => {
    ...
    setSaving(true);
    try {
      const response = await fetch(`${API_URL}/inbox/${selectedEmail.id}/reply`, {
        method: "POST",
        headers: authHeaders,
      ...
      setSelectedEmail((current) =>
        current ? { ...current, thread: [...(current.thread || []), data] } : current
      );
    } catch (err) {
  ...
  const sendEmail = async (asDraft = false) => {
    const toEmails = compose.to
      ...
      .filter(Boolean);
    if (!toEmails.length || !compose.subject.trim() || !compose.body.trim()) return;
    ...
    <div className="flex h-full w-full overflow-hidden rounded-xl bg-white shadow-md">
            ...
            return (
              <button
                ...
                className={`flex h-9 w-full items-center gap-3 rounded-md px-3 text-sm ${
                  folder === item.value ? "bg-stone-900 font-medium text-white" : "text-stone-700 hover:bg-stone-200"
                }`}
              >
      ...
      <section className="flex h-full w-[420px] shrink-0 flex-col border-r">
        ...
        <div className="flex-1 overflow-auto">
          {loading && <p className="p-4 text-sm text-stone-500">Loading mail...</p>}
          {!loading && emails.length === 0 && <p className="p-4 text-sm text-stone-500">No emails in this folder.</p>}
          {emails.map((email) => (
            <button
              key={email.id}
            ...
            >
              <div className="mb-1 flex items-center gap-2">
                ...
                >
                  <Star className={`h-4 w-4 ${email.isStarred ? "fill-yellow-400 text-yellow-500" : "text-stone-400"}`} />
                </button>
                <p className={`min-w-0 flex-1 truncate text-sm ${email.isRead ? "font-medium" : "font-bold"}`}>
                  {folder === "sent" ? email.toEmails.join(", ") : `${email.fromName} <${email.fromEmail}>`}
                </p>
                {email.hasAttachment && <Paperclip className="h-4 w-4 text-stone-400" />}
              </div>
              <p className={`truncate text-sm ${email.isRead ? "font-medium" : "font-bold"}`}>{email.subject}</p>
              <p className="mt-1 line-clamp-2 text-xs leading-5 text-stone-500">{email.snippet}</p>
          ...
          <>
            <header className="flex min-h-16 items-center gap-2 border-b px-5">
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => patchEmail(selectedEmail.id, { isRead: !selectedEmail.isRead })}
                disabled={saving}
              >
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => patchEmail(selectedEmail.id, { folder: "archive" })}
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => patchEmail(selectedEmail.id, { folder: "trash" })}
              ...
              {selectedThread.map((message) => (
                <article key={message.id} className="mb-4 rounded-lg border bg-white p-4 shadow-sm">
                  <div className="mb-4 flex items-start gap-3">
                    ...
                    <span className="text-xs text-stone-500">{formatTime(message.sentAt)}</span>
                  </div>
      ...
      {showCompose && (
        <div className="absolute bottom-6 right-6 z-20 flex w-[520px] flex-col overflow-hidden rounded-lg border bg-white shadow-2xl">
          ...
          <div className="flex justify-between border-t p-3">
            <Button variant="outline" onClick={() => sendEmail(true)} disabled={saving}>
              Save draft
            </Button>
