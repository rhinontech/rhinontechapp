...
async function seed() {
    ...
    const permissions = await Promise.all(ALL_PERMISSIONS.map((p) => models_1.Permission.findOrCreate({ where: { name: p.name }, defaults: p })));
    const permissionInstances = permissions.map(([p]) => p);
    console.log("Permissions seeded");
    // Create superadmin role with all permissions
    const [superadminRole] = await models_1.Role.findOrCreate({
        where: { slug: "superadmin" },
        defaults: { name: "Super Admin", slug: "superadmin" },
    });
    await superadminRole.setPermissions(permissionInstances);
    ...
    const conversationSeeds = [
        {
            ...
            messages: [
                { senderType: "customer", senderName: "Aarav Mehta", body: "Hi, I connected our WhatsApp Business number but messages are not showing in the inbox." },
                { senderType: "agent", senderName: "Ishra Fatima", body: "Thanks Aarav. I am checking the webhook configuration now." },
                { senderType: "agent", senderName: "Ishra Fatima", body: "Can you confirm if the WhatsApp webhook is active on your side?" },
            ],
        },
        {
            ...
            messages: [
                { senderType: "customer", senderName: "Neha Kapoor", body: "The March invoice opens a blank page after I click download." },
                { senderType: "system", senderName: "Rhinon", body: "Conversation routed to Billing." },
            ],
        },
        {
            ...
            messages: [
                { senderType: "customer", senderName: "Kabir Shah", body: "We need to rotate our production API token urgently." },
                { senderType: "agent", senderName: "Ishra Fatima", body: "I have verified ownership. Generating a replacement token now." },
                { senderType: "agent", senderName: "Ishra Fatima", body: "We rotated the token and invalidated the old key." },
            ],
        },
        {
            ...
            messages: [
                { senderType: "customer", senderName: "Sara Thomas", body: "Can I invite teammates during the trial?" },
                { senderType: "agent", senderName: "Super Admin", body: "Yes. Trial workspaces can invite teammates from settings." },
                { senderType: "customer", senderName: "Sara Thomas", body: "Thanks, that clears it up." },
            ],
        },
    ];
    for (const seedConversation of conversationSeeds) {
        ...
        const [conversation] = await models_1.InboxConversation.findOrCreate({
        ...
        const messageCount = await models_1.InboxMessage.count({ where: { conversationId: conversation.id } });
        if (messageCount === 0) {
            await models_1.InboxMessage.bulkCreate(messages.map((message, index) => ({
    ...
    for (const email of emailSeeds) {
        await models_1.InboxEmail.findOrCreate({
